...
state="${workspace}/.panthera-phone-refined-target-assist-pipeline-state"

if [[ -d "$state" ]] && find "$state" -mindepth 1 -print -quit | grep -q .; then
  echo "错误：低冲击末端技能流水线状态目录非空：${state}" >&2
  exit 1
fi
...
mkdir -p "$state"
nohup bash "${workspace}/run_lab_panthera_phone_refined_target_assist_pipeline.sh" \
