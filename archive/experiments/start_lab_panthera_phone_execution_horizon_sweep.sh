...
state="${workspace}/.panthera-phone-execution-horizon-sweep-state"

if [[ -d "$state" ]] && find "$state" -mindepth 1 -print -quit | grep -q .; then
  echo "错误：执行视野 sweep 状态目录非空：${state}" >&2
  exit 1
fi
...
mkdir -p "$state"
nohup bash "${workspace}/run_lab_panthera_phone_execution_horizon_sweep.sh" \
