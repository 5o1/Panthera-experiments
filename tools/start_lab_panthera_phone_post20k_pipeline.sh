...
state_root="${workspace}/state/panthera-phone-post20k-pipeline-state"
...
if [[ -s "${state_root}/launcher-pid.txt" ]]; then
  ...
  if [[ "$old_pid" =~ ^[0-9]+$ ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "错误：20k 后续评测流水线已经在运行（PID ${old_pid}）。" >&2
    exit 1
  fi
fi
if [[ -f "${state_root}/pipeline.ok" ]]; then
  echo "20k 后续评测流水线已经通过。"
  exit 0
fi

nohup bash "${workspace}/bin/run_lab_panthera_phone_post20k_pipeline.sh" \
