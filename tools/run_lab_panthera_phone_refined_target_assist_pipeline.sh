...
# The former held-out set is now an explicit development gate because its
# failure modes informed this lower-impact insertion refinement.
run_gate \
  policy-eval-25x7-refined-target-assist-dev16 \
  "${workspace}/panthera-rlinf-overlay/seeds/panthera_phone_vertical_eval_seeds.json"

# This set remains unseen until the development gate passes.
run_gate \
  policy-eval-25x7-refined-target-assist-final16 \
  "${workspace}/panthera-rlinf-overlay/seeds/panthera_phone_vertical_final_seeds_v2.json"

date --iso-8601=seconds >"${workspace}/.panthera-phone-refined-target-assist-gates.ok"
