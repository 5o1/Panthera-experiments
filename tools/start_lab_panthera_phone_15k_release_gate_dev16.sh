...
state="${workspace}/.panthera-phone-15k-release-gate-h18-dev16-state"

if [[ -d "$state" ]] && find "$state" -mindepth 1 -print -quit | grep -q .; then
  echo "错误：15k 延迟接管开发集状态目录非空：${state}" >&2
  exit 1
fi
...
mkdir -p "$state"
nohup bash "${workspace}/run_lab_panthera_phone_15k_release_gate_dev16.sh" \
