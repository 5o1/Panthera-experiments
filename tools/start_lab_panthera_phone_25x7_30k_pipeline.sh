...
state="${workspace}/.panthera-phone-25x7-30k-pipeline-state"

if [[ -d "$state" ]] && find "$state" -mindepth 1 -print -quit | grep -q .; then
  echo "错误：25x7 30k 流水线状态目录非空：${state}" >&2
  exit 1
fi
...
mkdir -p "$state"
nohup bash "${workspace}/run_lab_panthera_phone_25x7_30k_pipeline.sh" \
