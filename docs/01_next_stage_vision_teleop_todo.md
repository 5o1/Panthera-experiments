...
- [ ] step 10000 不松爪诊断：已重跑并记录 53 个完整 25×7 预测块。策略在 row 140 首次
      ...
      action，故已排除 20/25 执行视野截断。专家从 row 790 开始连续增大夹爪开度，并在
      row 801 越过诊断及释放接触配置使用的 0.5 中点；模型到达插入阶段后仍持续输出约
      0.3 的闭合值。下一步应定位模型为何没有生成连续释放轨迹，而不应先改执行器逻辑或
      强制开爪。
- [ ] step 45000 的逐时间步离线 L1 排除了“模型根本没学会松爪”。在记录的专家帧上，
      ...
      反而低于全局均值，因此只看聚合 loss 会掩盖释放困难。这里的 0.5 用于诊断二值化，
      执行器还用它识别首次重开并配置释放接触求解，但夹爪动作本身仍是连续开度；
      专家目标从 row 790 的 `0.30704` 起连续增大，row 801 越过 0.5，row 824 达到
      `0.90503`，并非相邻帧突然给出语义相反动作。自主闭环中，同一 checkpoint
      ...
      `3.36e-8 m` 一致性检查，因此不是相机错位。专家 row 800→801 的整幅图平均变化仅
      `0.20/255`，状态差仅 `4.65e-5`，但对应标签也是连续开爪斜坡的相邻值，而且相邻
      25 步动作块共享 24 步；该数据不能单独证明阶段混叠。当前已确认的是闭环协变量偏移后
      开爪轨迹未触发；更合适的视角、时间历史或显式阶段状态仍只是待验证候选。
      曲线、CSV 和摘要在 `artifacts/step-45000-temporal-loss/`，Lab 原件在该运行目录的
      `paired-preview-step-45000/standard-h20/`。
- [x] 2026-09-24 按用户视频复核修正阶段归因：当前不稳定的前段是**有效夹持形成之前**，
      机械臂在圆柱附近的严重抖动会先碰倒并推离圆柱，因而“稳定抓取和抬升”尚未解决；
      不能把这类失败表述成抓住后的搬运偏离。以“已经稳定夹住圆柱”为条件，现有人工审阅
      的闭环样本随后都能完成搬运并到达槽上方。因而当前应把问题拆成两个瓶颈：抓取前的
      近物体抖动/碰撞，以及到位后的松爪转移；中间持物搬运是当前证据下的条件稳定段。
      这里的“一定”仅指已经审阅的 rollout，不外推为未见场景的形式保证。
- [x] 补充搬运—释放差异的原始文献证据并保存 6 篇 PDF：接触动力学把建立/解除接触视为
      不连续模式变化；PerAct 把瓶颈停点和夹爪状态突出为关键动作；CLIPort/MimicGen 用
      pick-place 抽象或 termination signal 表示子任务边界；DAgger/ACT 分别解释闭环分布
      偏移和长时域误差。文献支持当前消融方向，但没有直接证明 Panthera 的释放失败原因。
      索引见 [搬运—释放论文索引](references/release_transition_papers.md)。
- [x] step 65000 的滑动平均版经用户视频审阅，已经能稳定、准确地把圆柱送到槽点正上方，
      ...
      分界时，RGB 平均变化仅 `0.200/255`、7 维状态 L2 仅 `4.29e-5`；模型在专家帧上能
      沿连续轨迹打开，在自主闭环轻微偏移下却停在闭合固定点。结论只能确认闭环分布偏移后
      开爪未触发；单帧阶段混叠仍是假设，而不是由这对相邻帧证明的事实。已排除相机位姿
      错配、执行视野截断和滑动平均压制开爪。证据见
...
- [x] 完成层级技能、终止函数和外部状态验证的原始论文检索，并在仓库保存 7 篇 PDF、来源
      和 SHA-256。文献支持把 `grasp → move_held_object → release` 列为最优先消融，但不
      支持预先宣称必然改善；最接近的 planner-monitor 系统也报告了视觉监视器过早确认。
      具体证据和四级对照实验见
      [层级技能转移文献调查](26_hierarchical_skill_transition_literature_review_2026-09-24.md)。
- [ ] 运行层级技能最小消融：依次比较 Flat、阶段条件+oracle 转移、学习转移申请+oracle
      验证、学习转移申请+可部署验证器；固定 episode、checkpoint、场景和动作执行模式，
      分别报告到达释放区、松爪、完整任务、过早切换和技能振荡，不能只报告聚合 L1。
- [x] 完成连续可接受状态与多解动作分布的跨领域检索并保存 16 篇 PDF、来源和 SHA-256。
      结论是先学习状态—动作成功概率场 `g(s,a)`，再导出可接受释放状态和可行动作超水平集，
      最后在集合内部学习条件轨迹分布；有限专家点不能识别完整可行域，必须增加仿真/规划器
      结果标签或同一邻域的多个成功与失败后继。初版综述因没有逐篇说明问题定义、训练数据、
      模型输出、损失和推理流程被用户否决；第二版已按“只改模型与损失／训练时数据变换／
      新增可行性监督”三类重写，并补充 Panthera 查询数据 schema。研究报告和 F0--F4 对照见
      [连续可行域研究](28_continuous_feasible_state_action_distribution_research_2026-09-24.html)。
- [ ] 在任何生成式动作头重训前，先建立 release feasibility arena：连续扰动相对位姿、
      姿态、速度、夹爪和接触状态，执行事件驱动的释放—沉降检查，训练并校准
      `P(success | state, release)`；特权状态上无法在未见扰动中通过校准门禁时，不进入
      RGB+28 维 proprio 或多解轨迹训练。
- [x] 完成释放切换强化学习奖励的原始文献检索并保存 12 篇 PDF、来源和 SHA-256。主实验
      不直接奖励夹爪动作或专家阶段，而把成功定义为合法区域解除接触后圆柱稳定入槽；
      使用终局稀疏奖励、效果型势函数差分和显式 Reward Machine 三组同口径对照。反向课程
      同时采样合法释放位、近但不合法的位置和搬运途中，防止策略学成“一开始就开爪”。
      方案、量级、反作弊测试和指标见
      [强化学习奖励设计](27_openvla_rl_release_reward_design_2026-09-24.md)。
- [ ] 在长时间 RL 前完成五道门禁：reward evaluator 六类脚本单测；专家/失败回放的回报
      排序；连续 25×7 随机策略及 action-level log-prob；双 rank 非零且同步的策略梯度；
      重叠 chunk 的绝对时刻对齐单测。
      2026-09-24 已完成七类纯 reward 反作弊测试、连续 25×7 action-level log-prob、双 rank
      非零 PPO 梯度和重叠对齐单测；Lab 正式 runtime 为 RoboTwin `11 passed`、连续 PPO
      `10 passed`。仍缺真实专家/失败物理回放的回报排序，故本项不提前勾选。原先连续分支
      `chunk_logprobs=0` 的限制已由冻结 SFT 均值周围的有界 Gaussian residual policy 取代。
- [ ] 增加相邻查询的旧 chunk 条件与一致性门禁：按实际执行间隔对齐同一绝对时刻，只
      输入真实执行历史、有效旧尾部及 mask，并排除 episode/reset/终止边界和过期计划。
      smoke 比较无一致性 `C0`、只有六关节 loss 的 `C1`、历史+旧尾部+loss+历史扰动的
      `C2`；夹爪一致性 `C3` 仅作低权重消融，避免把约 0.30 的闭合固定点进一步固化。
      必须同时报告重叠预测误差、chunk 边界 jerk、释放触发延迟和任务成功率；
      恒定策略的一致性虽高但任务失败，不得把一致性指标单独当成通过。R1 工程 smoke 的
      C0/C1/C2 已各完成一次有限 PPO 更新并落盘。三者 release-arena return 分别为
      `0.07097/0.12410/0.08633`，`grad_norm=0.833/1.006/1.137`，但成功率都为 0。首轮 C2
      暴露 float32 rollout history 进入 bf16 history encoder 的 dtype 错配；修复并增加
      backward 回归后，真实 rollout 与 checkpoint 均通过。由于一步 return 的 trajectory
      数不同，不能据此选模；各 10 次更新的同预算 C0/C1/C2 对照正在物理 GPU 2、3 串行运行。
      已新增 checkpoint-only deterministic release-arena 评测入口；它不会追加 PPO update，
      会保存每组视频和指标，并已排在本轮训练 COMPLETE 后自动串行执行。十更新 C0 已以
      退出码 0 完成，10 条 TensorBoard 指标均有限但训练 success 全为 0；C1 也以退出码 0
      完成 10/10 次更新，10 条指标均有限，第 2 次更新的两条 stochastic 轨迹中有一条成功，
      其余九次未重复，不能替代独立评测。C2 已按 `noise=0.01`、`dropout=0.10` 自动接续，
      仍只使用物理 GPU 2、3。正式评测遥测已补齐可靠夹持、持物到位、合法／非法释放、
      到位仍夹持、硬失败、释放延迟及其 censor 标记，以及动作一阶变化、二阶差分和 chunk
      边界二阶差分；这些特权量只进入 `info`，不进入策略 observation 或 reward。RoboTwin
      回归 `12 passed`，RLinf 标量宽度／传递合同与汇总器测试通过；增强版 R0/C0/C1/C2
      评测已替换旧的纯等待队列，训练本身未中断。
- [ ] 从 DDP 修复后的 `step 75000` 构建冻结主干的有界残差策略，先跑 release arena 的
      `R0/R1/R2`（SFT、终局奖励、势函数奖励），再向完整任务反向扩展；显式阶段 `R3`
      只作为诊断上界。每组至少三个训练 seed，并报告有效夹持、条件搬运、合法释放、
      提前释放、到位仍夹持、稳定入槽、碰撞/抖动和基准策略漂移。release arena 已能从
      episode 2 前 780 个专家动作构造 reset，且 R2-C0/C1/C2 均完成首个真实 PPO update；
      首跑暴露
      bf16 `atanh(±1)` 的 NaN，修复为先升 float32 后 clamp，并增加非有限值 fail-fast。
      三组修复后均有非零 return 且 loss/gradient 有限，但 `success_once=0`，当前只进入
      10 更新短程对照，尚不能进入完整任务课程。详细证据见
      [release arena RL smoke](29_openvla_release_arena_rl_smoke_2026-09-24.md)。
      冻结 SFT R0 的无 checkpoint 入口已加入严格互斥保护：R0 必须显式选择且禁止传入
      checkpoint，RL 评测则必须显式提供 checkpoint。零残差等价测试 `10 passed`、launcher
      测试 `5 passed`、含 R0 的汇总测试 `2 passed`，独立临时 runtime 也通过装配和 Hydra
      解析；R0 rollout 和三种 PhysX reward replay 已由等待型 systemd 服务排在本轮独立
      评测之后，相关后置脚本在 Lab 合计 `9 passed`，等待不占 GPU、执行只使用物理 GPU
      2、3。实际 R0 rollout 与稀疏 R1 对照仍未完成，因此本项保持未完成。
