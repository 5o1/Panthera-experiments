...
目标 0.3。专家标签在 row 801 从 `0.48291` 上升到 `0.50050` 开始松开。因此当前直接原因
是 step 10000 模型没有输出松爪 action，不是执行器吞掉了已经生成的松爪指令。诊断产物在
...
部分可观测：专家松爪阈值前后的 row 800→801 整幅图平均变化仅 `0.20/255`，proprio
状态差仅 `4.65e-5`；当前策略又只输入单张 RGB 和当前 proprio，没有时间历史、接触或
“已稳定多久”的状态。结论是闭环协变量偏移和阶段混叠为当前直接解释；新增侧视/腕部
视角、帧历史或显式接触/阶段变量属于待验证的观测改进，单独移动固定相机并非已证实修复。
...
专家数据在释放切换处存在单帧阶段混叠：HDF5 source row 812 到 813 的夹爪 action 从
`0.482915` 跨到 `0.500503`，但整幅 RGB 平均像素变化只有 `0.199852/255`，7 维 robot
state 的 L2 变化只有 `4.29e-5`。同一 checkpoint 在记录帧上 row 802--803 能输出
`0.506445 -> 0.528320`，证明权重不是完全没有学到开爪；在自主闭环的轻微分布偏移下则
停在约 `0.30` 的闭合固定点。当前证据把直接原因收敛为：单帧 RGB + 当前 proprio 不包含
“已在释放位稳定多久”的阶段信息，而示范在几乎相同观测下先保持闭合、随后打开。下一步
应比较最小时间历史/阶段转移输出，不得用槽口几何规则强制开爪来冒充 VLA 成功。
