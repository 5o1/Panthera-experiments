...
def _task_config_root(robotwin_root: Path) -> Path:
    """Resolve RoboTwin's current config layout, retaining old-data compatibility."""
    for relative in ("env_cfg/task_config", "task_config"):
        candidate = robotwin_root / relative
        if candidate.is_dir():
            return candidate
    raise FileNotFoundError(f"RoboTwin task config directory is missing: {robotwin_root}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=Path(
            os.environ.get(
                "ROBOTWIN_PATH", "/data/lyy/panthera-vla/runtime/robotwin"
            )
        ),
    )
    parser.add_argument(
...
def _load_task_args(robotwin_root: Path, task_config: str, task_name: str) -> dict:
    ...
    config_root = _task_config_root(robotwin_root)
    config_path = config_root / task_config
    with config_path.open("r", encoding="utf-8") as stream:
        task_args = yaml.safe_load(stream)

    embodiment = task_args["embodiment"]
    with (config_root / "_embodiment_config.yml").open(
        "r", encoding="utf-8"
    ) as stream:
