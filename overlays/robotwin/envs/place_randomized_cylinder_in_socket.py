...
from .panthera_camera_randomization import (
    camera_randomization_spec,
    sample_constrained_camera,
)
from .panthera_scene_registry import load_registry, validate_scene
from .panthera_v2_sampling import sample_scene

# Registries are read once per path and shared by every episode in the process,
# because the collector and the evaluator both rebuild the task per episode.
_SCENE_REGISTRIES: dict[str, dict[int, dict]] = {}
...
class place_randomized_cylinder_in_socket(place_vertical_cylinder_in_groove):
    ...
    def setup_demo(self, **kwargs):
        ...
        self._apply_constrained_camera_randomization()
        rigid_body = next(
            component
    ...
    def _camera_visibility_keypoints(self) -> tuple[list[str], np.ndarray]:
        ...
        def add(name: str, point) -> None:
            value = np.asarray(point, dtype=float)
            if value.shape != (3,) or not np.all(np.isfinite(value)):
                raise ValueError(f"invalid camera keypoint {name}")
            names.append(name)
            points.append(value.tolist())

        for index, link in enumerate(self.robot.left_entity.get_links()):
            add(f"robot_link_{index}_{link.get_name()}", link.get_pose().p)

        geometry = self.realized_geometry
        workspace = geometry["workspace"]
        base_x = float(workspace["robot_base_x_m"])
        base_y = float(workspace["robot_base_y_m"])
        radius = float(workspace["maximum_radius_m"])
        # This bounding volume covers the robot base, the 75%-reach task
        # semicircle and the elevated carry route, not just the initial pose.
        for x_index, x_value in enumerate((base_x - radius, base_x + radius)):
            for y_index, y_value in enumerate((base_y, base_y + radius)):
                for z_index, z_value in enumerate(
                    (TABLE_HEIGHT_M, TABLE_HEIGHT_M + 0.46)
                ):
                    add(
                        f"robot_task_envelope_{x_index}{y_index}{z_index}",
                        [x_value, y_value, z_value],
                    )

        cylinder_center = np.asarray(self.cylinder.get_pose().p, dtype=float)
        cylinder_axis = np.asarray(self.initial_cylinder_axis_world, dtype=float)
        cylinder_axis /= np.linalg.norm(cylinder_axis)
        add("cylinder_center", cylinder_center)
        add(
            "cylinder_axis_negative",
            cylinder_center - cylinder_axis * CYLINDER_HALF_HEIGHT_M,
        )
        add(
            "cylinder_axis_positive",
            cylinder_center + cylinder_axis * CYLINDER_HALF_HEIGHT_M,
        )
        radial = np.cross(cylinder_axis, np.array([0.0, 0.0, 1.0]))
        if np.linalg.norm(radial) <= 1.0e-8:
            radial = np.array([1.0, 0.0, 0.0])
        radial /= np.linalg.norm(radial)
        add("cylinder_radius_negative", cylinder_center - radial * CYLINDER_RADIUS_M)
        add("cylinder_radius_positive", cylinder_center + radial * CYLINDER_RADIUS_M)

        socket_xy = np.asarray(geometry["socket_target_xy_m"], dtype=float)
        socket_half_width = SOCKET_INNER_HALF_WIDTH_M + SOCKET_WALL_HALF_WIDTH_M
        socket_half_length = CYLINDER_RADIUS_M + SOCKET_WALL_HALF_WIDTH_M
        socket_top = (
            TABLE_HEIGHT_M
            + 2.0 * SOCKET_FLOOR_HALF_HEIGHT_M
            + 2.0 * SOCKET_WALL_HALF_HEIGHT_M
        )
        for x_index, dx in enumerate((-socket_half_length, socket_half_length)):
            for y_index, dy in enumerate((-socket_half_width, socket_half_width)):
                for z_index, z_value in enumerate((TABLE_HEIGHT_M, socket_top)):
                    add(
                        f"socket_bound_{x_index}{y_index}{z_index}",
                        [socket_xy[0] + dx, socket_xy[1] + dy, z_value],
                    )
        return names, np.asarray(points, dtype=float)

    def _apply_constrained_camera_randomization(self) -> None:
        """Set one replayable overhead pose after all critical actors exist."""
        config = self.task_randomization.get("camera_randomization", {}) or {}
        spec = camera_randomization_spec(config)
        if not spec.enabled:
            self.camera_randomization_audit = {
                "schema_version": 1,
                "enabled": False,
            }
            return
        names, points = self._camera_visibility_keypoints()
        bounds_min = np.min(points, axis=0)
        bounds_max = np.max(points, axis=0)
        center = (bounds_min + bounds_max) / 2.0
        head_camera = self.cameras.static_camera_list[self.cameras.head_camera_id]
        # A fixed pose cannot change between attempts, so the render gate is
        # evaluated once instead of being resampled.
        visibility_attempts = 1 if spec.pose_mode == "fixed" else 64
        for visibility_attempt in range(visibility_attempts):
            audit = sample_constrained_camera(
                self.episode_seed,
                TABLE_HEIGHT_M,
                center,
                names,
                points,
                config,
                sampling_stream=visibility_attempt,
            )
            rotation = np.column_stack(
                (
                    np.asarray(audit["forward_xyz"], dtype=float),
                    np.asarray(audit["left_xyz"], dtype=float),
                    np.asarray(audit["up_xyz"], dtype=float),
                )
            )
            transform = np.eye(4, dtype=float)
            transform[:3, :3] = rotation
            transform[:3, 3] = np.asarray(audit["position_xyz_m"], dtype=float)
            head_camera.entity.set_pose(sapien.Pose(transform))
            self.scene.update_render()
            head_camera.take_picture()
            segmentation = head_camera.get_picture("Segmentation")[..., 1]
            visibility = self._rendered_camera_visibility(
                segmentation,
                audit["central_region_uv"],
            )
            if visibility["passed"]:
                audit["enabled"] = True
                audit["rendered_visibility"] = visibility
                audit["visibility_attempt"] = visibility_attempt + 1
                self.camera_randomization_audit = audit
                return
        raise RuntimeError(
            "could not sample a camera with robot, cylinder and socket visibly centered"
        )

    def _rendered_camera_visibility(
        self,
        actor_labels: np.ndarray,
        central_region_uv: list[float],
    ) -> dict:
        """Require rendered pixels for every critical actor group in the crop."""
        labels = np.asarray(actor_labels)
        if labels.ndim != 2:
            raise ValueError("actor segmentation must be a 2-D label image")
        height, width = labels.shape
        lower_u, lower_v, upper_u, upper_v = map(float, central_region_uv)
        x0 = int(math.ceil(lower_u * width))
        x1 = int(math.floor(upper_u * width))
        y0 = int(math.ceil(lower_v * height))
        y1 = int(math.floor(upper_v * height))
        central_mask = np.zeros_like(labels, dtype=bool)
        central_mask[y0:y1, x0:x1] = True

        robot_ids = [
            int(link.entity.get_per_scene_id())
            for link in self.robot.left_entity.get_links()
        ]
        cylinder_ids = [int(self.cylinder.actor.get_per_scene_id())]
        socket_entities = [self.groove_base, *self.socket_walls]
        socket_ids = [
            int(entity.actor.get_per_scene_id()) for entity in socket_entities
        ]
        groups = {
            "robot": (robot_ids, 200),
            "cylinder": (cylinder_ids, 6),
            "socket": (socket_ids, 20),
        }
        result = {}
        passed = True
        for name, (actor_ids, minimum_pixels) in groups.items():
            visible = np.isin(labels, actor_ids)
            total_pixels = int(np.count_nonzero(visible))
            central_pixels = int(np.count_nonzero(visible & central_mask))
            group_passed = (
                total_pixels >= minimum_pixels and central_pixels == total_pixels
            )
            result[name] = {
                "actor_ids": actor_ids,
                "visible_pixels": total_pixels,
                "central_region_pixels": central_pixels,
                "minimum_visible_pixels": minimum_pixels,
                "all_visible_pixels_in_central_region": central_pixels == total_pixels,
                "passed": group_passed,
            }
        ...
        return {
            "schema_version": 1,
            "passed": passed,
            "central_region_pixel_bounds_xyxy": [x0, y0, x1, y1],
            "groups": result,
        }
    ...
    def _retime_joint_path_by_arc_length(
        ...
        for iteration in range(1, CONTINUOUS_RETIME_MAX_ITERATIONS + 1):
            ...
            # 收敛判据必须与数据集审计门禁同量纲。审计按值空间比较 0.1%
            # （速度 <= 0.601、加速度 <= 2.002）；用 scale <= 1.001 等于允许
            # 加速度到 2 * 1.001^2 = 2.004002，会产出被审计否决的轨迹。
            if (
                peak_velocity <= CONTINUOUS_MAX_JOINT_VELOCITY_RADPS * 1.001
                and peak_acceleration
                <= CONTINUOUS_MAX_JOINT_ACCELERATION_RADPS2 * 1.001
            ):
                break
            duration *= scale * 1.02
        else:
    ...
    def _registered_scene(self) -> dict | None:
        """Return the recorded geometry for this seed, when one is registered.

        A seed does not identify a scene: collection overrode the seed's posture
        and lying angle per shard and kept only the outcome, so rebuilding from
        the seed alone lands on a different scene for about half the episodes.
        A registry restores the recorded geometry directly, which removes the
        need for any caller to reconstruct the forcing flags or re-verify the
        result by hand.
        """
        path = self.task_randomization.get("scene_registry")
        if not path:
            return None
        resolved = str(Path(path).expanduser())
        registry = _SCENE_REGISTRIES.get(resolved)
        if registry is None:
            registry = load_registry(resolved)
            _SCENE_REGISTRIES[resolved] = registry
        scene = registry.get(int(self.episode_seed))
        if scene is None:
            if self.task_randomization.get("scene_registry_required", True):
                raise ValueError(
                    f"seed {self.episode_seed} is not in {resolved}; set "
                    "scene_registry_required: false to fall back to sampling"
                )
            return None
        validate_scene(
            scene,
    ...
    def _sample_task_scene(self, rng: np.random.Generator) -> dict:
        ...
        del rng
        self.scene_source = "sampled"
        registered = self._registered_scene()
    ...
    def _finalize_episode_info(self):
        ...
        # Provenance: whether this scene came from the sampler or from a
        # registry, so a later replay can tell which contract it must restore.
        episode["scene_source"] = getattr(self, "scene_source", "sampled")
        ...
        episode["camera_randomization"] = deepcopy(
            self.camera_randomization_audit
        )
