...
def update_obs(observation, proprio_key="vector", proprio_dim=None):
    ...
    if proprio_key in robot_state:
        state = np.asarray(robot_state[proprio_key])
    elif proprio_key == "vector":
        # Generic upstream tasks do not expose Panthera's measured
        # ``robot_state`` mapping.  Keep their historical qpos fallback while
        # requiring dynamics-aware tasks to satisfy the explicit contract.
        state = np.asarray(observation["joint_action"]["vector"])
    else:
        raise RuntimeError(
            f"robot observation has no requested proprio field {proprio_key!r}"
        )
    if proprio_dim is not None and state.shape != (int(proprio_dim),):
        raise RuntimeError(
            f"proprio field {proprio_key!r} has shape {state.shape}, "
            f"expected ({int(proprio_dim)},)"
        )
...
class SubEnv:
    def __init__(
        ...
        self.args = args
        self.proprio_key = str(args.get("proprio_key", "vector"))
        configured_proprio_dim = args.get("proprio_dim")
        self.proprio_dim = (
            None if configured_proprio_dim is None else int(configured_proprio_dim)
        )
    ...
    def step(self, actions):
        ...
        with self.lock:
            ...
            obs = update_obs(
                self.task.get_obs(), self.proprio_key, self.proprio_dim
            )
            # ``self.instruction`` is the language command generated for this
            # exact reset.  Newer RoboTwin task classes do not necessarily
            # mirror it back through ``task.get_instruction()``, even though
            # it is passed to ``setup_demo`` in ``self.args``.  Returning the
            # task-side value therefore produced ``None`` prompts in RLinf.
    ...
    def get_obs(self):
        with self.lock:
            ...
            obs = update_obs(obs, self.proprio_key, self.proprio_dim)
            obs["instruction"] = self.instruction
...
class VectorEnv(gym.Env):
    def __init__(
        ...
        args["n_envs"] = n_envs
        args["action_dim"] = int(args.get("action_dim", 14))
        if args["action_dim"] <= 0:
            raise ValueError("task_config.action_dim must be positive")
